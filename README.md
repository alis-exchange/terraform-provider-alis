...
Tests come in three tiers, from hermetic to cloud-backed:
...
| `ALIS_OS_PROJECT` | Integration | Google Cloud project for live runs (used only when no emulator is available) |
| `ALIS_OS_INSTANCE` | Integration | Spanner instance for live runs (used only when no emulator is available) |
| `ALIS_OS_DATABASE` | Integration | Existing live database the lifecycles run inside (default `tf-test`) |

`make testacc` (the `GNUmakefile` default) runs the whole suite with `TF_ACC=1` and a 120-minute timeout.
